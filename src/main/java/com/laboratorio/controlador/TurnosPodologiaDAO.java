...
public class TurnosPodologiaDAO {
    ...
    public List<TurnoPodologia> listarPorFecha(String fecha) {
        List<TurnoPodologia> lista = new ArrayList<>();
        String sql = "SELECT id, cedula, nombres, celular, email, motivo, fecha, " +
    ...
    public boolean guardarOActualizar(TurnoPodologia turno) {
        ...
        if (esEdicion) {
            sql = "UPDATE turnos_podologia SET cedula=?, nombres=?, celular=?, email=?, motivo=?, fecha=?, hora_inicio=?, duracion_minutos=? WHERE id=?";
        } else {
            sql = "INSERT INTO turnos_podologia (cedula, nombres, celular, email, motivo, fecha, hora_inicio, duracion_minutos) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        }
